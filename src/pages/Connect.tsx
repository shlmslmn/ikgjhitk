...
export default function Connect() {
  ...
  // Real-time subscriptions for follow status changes from other pages
  useEffect(() => {
    if (!user?.id) return;
  ...
  const loadMembers = async () => {
    try {
      const { data, error } = await supabase
        .from('profiles')
        .select('id, name, avatar_url, tier, followers, bio')
        .eq('account_type', 'member')
        .limit(50);

      if (!error && data) {
        // Check which members the user is connected with
        const { data: connectedData } = await supabase
          .from('member_connections')
          .select('connected_user_id')
          .eq('member_id', user?.id);

        const connectedIds = new Set(connectedData?.map(c => c.connected_user_id) || []);
  ...
  const handleConnectMember = async (member: Member) => {
    try {
      ...
      if (!error) {
        // Update local state - connection is recorded in database
        setMembers(
          members.map((m) =>
            m.id === member.id ? { ...m, connected: true } : m
          )
        );

        addToast(`Connected with ${member.name}`, 'success');

        // Reload members after 1 second to get updated data from database
